<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="docblock">
        ...
        <xsl:for-each select="$ctx/pdx:docblock">
            <ul>
                <xsl:if test="pdx:file">
                    <li>File: <xsl:value-of select="pdx:file/@value" /></li>
                </xsl:if>
                ...
                <xsl:if test="pdx:since">
                    <li>Since: <xsl:value-of select="pdx:since/@value" /></li>
                </xsl:if>
                <xsl:if test="pdx:date">
                    <li>Date: <xsl:value-of select="pdx:date/@value" /></li>
                </xsl:if>
            </ul>
        </xsl:for-each>
    </xsl:template>
